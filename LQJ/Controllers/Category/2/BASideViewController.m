...
@interface BASideViewController ()<UIGestureRecognizerDelegate>
{
    CGFloat _screenWidth;   // 屏幕宽度
    CGFloat _sccreenHeight; // 屏幕高度
    CGPoint _viewStartCenterPoint; // self.view.center
    UIView *_tapView;       // Tap手势View
    CGPoint _beginPoint;    // 主VC初始center
    CGPoint _beginSidePoint; // 侧拉VC初始center
}

@end

@implementation BASideViewController

- (instancetype)initWithSideVC:(UIViewController *)sideMenuVC currentVC:(UIViewController *)currentMainVC
{
    UIViewController *rootVC = SharedApplication.delegate.window.rootViewController;
    ...
    if (self = [super init]) {
        ...
        _sccreenHeight = [UIScreen mainScreen].bounds.size.height;
        _viewStartCenterPoint = self.view.center;
        _currentVC = currentMainVC;
        _sideVC = sideMenuVC;
        _sideContentOffset = _screenWidth * 3 / 4;
        _currentVCPanEnableRange = 100;
        _isSide = YES;
        _beginPoint = self.view.center;
        CGPoint point = self.view.center;
        point.x = point.x - _sideContentOffset / 2;
        _beginSidePoint = point;
...
- (void)setUpViewControllers
{
    ...
    _sideVC.view.bounds = CGRectMake(0, 0, _screenWidth, _sccreenHeight);
    
    [self.view addSubview:_sideVC.view];
    _currentVC.view.frame = CGRectMake(0, 0, _screenWidth, _sccreenHeight);
    ...
    _tapView.frame = CGRectMake(0, 0, _screenWidth - _sideContentOffset, _sccreenHeight);
...
- (void)panGesture:(UIPanGestureRecognizer *)pan
{
    ...
    if (pan.state == UIGestureRecognizerStateChanged) {
        ...
        CGPoint tabBarVCCenter = _beginPoint;
        CGPoint sideVCCenter = _beginSidePoint;
        tabBarVCCenter.x = _beginPoint.x + movePoint.x;
        sideVCCenter.x = _beginSidePoint.x + ((movePoint.x * (_sideContentOffset / 2)) / _sideContentOffset);
        if (tabBarVCCenter.x >= _viewStartCenterPoint.x  && (_viewStartCenterPoint.x + _sideContentOffset) >= tabBarVCCenter.x) {
            _currentVC.view.center = tabBarVCCenter;
            _sideVC.view.center = sideVCCenter;
        }else if (_viewStartCenterPoint.x > tabBarVCCenter.x){
            _currentVC.view.center = self.view.center;
            CGPoint point = self.view.center;
            point.x = point.x - (_sideContentOffset / 2);
            _sideVC.view.center = point;
        }else if (tabBarVCCenter.x > (_viewStartCenterPoint.x + _sideContentOffset)) {
            CGPoint point = self.view.center;
            point.x = _viewStartCenterPoint.x + _sideContentOffset;
...
- (void)closeSideVC
{
    ...
    [UIView animateWithDuration:0.2 animations:^{
        ...
        _beginPoint = _currentVC.view.center;
        _beginSidePoint = _sideVC.view.center;
    }];
}
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch
{
    ...
    // 控制pan手势范围
    if ([gestureRecognizer isKindOfClass:[UIPanGestureRecognizer class]] && ((_sccreenHeight - self.tabBarController.tabBar.frame.size.height) >= beginPointInView.y)) {
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context
{
    if ([keyPath isEqualToString:@"center"]) {
        ...
        if (point.x == (_viewStartCenterPoint.x + _sideContentOffset)) {
            _tapView.hidden = NO;
        }else {
            _tapView.hidden = YES;
        }
...
- (void)setSideContentOffset:(CGFloat)sideContentOffset
{
    ...
    _sideVC.view.frame = CGRectMake(- _sideContentOffset / 2, 0, _screenWidth, _sccreenHeight);
    _currentVC.view.frame = CGRectMake(0, 0, _screenWidth, _sccreenHeight);
    _tapView.frame = CGRectMake(0, 0, (_screenWidth - _sideContentOffset), _sccreenHeight);
}
